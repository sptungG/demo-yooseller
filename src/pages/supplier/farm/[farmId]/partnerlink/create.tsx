...
import PartnerLinkForm from "@/components/form/PartnerLinkForm";
...
import { useCreatePartnerLinksMutation } from "@/redux/query/pageprivate.query";
...
const Page = () => {
  ...
  const [createMutate, { isLoading: isLoadingCreateItem }] = useCreatePartnerLinksMutation();
  const handleCreateItem = (formValues: any) => {
    const { imageUrl, link, name, stt, providerId } = formValues as any;
    createMutate({
      imageUrl,
      link,
      name,
      ...
      .then(() => {
        notification.success({
          message: i18n["Tạo liên kết thành công"],
          placement: "bottomRight",
        });
        form.resetFields();
        replace(`/supplier/farm/${farmId}/partnerlink`);
      })
      .catch(() => {
        notification.error({
          message: i18n["Đã có lỗi xảy ra khi tạo liên kết"],
          placement: "bottomRight",
        });
      });
  };
      ...
      headerLeft={
        <BreadcrumbHeader
          items={[
            ...
            { [`/supplier/farm/${farmId}/partnerlink`]: i18n["Liên kết"] },
          ]}
          current={i18n["Thêm liên kết"]}
        />
      }
    >
      <PageWrapper>
