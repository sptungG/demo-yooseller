...
import useAmenityColumns from "@/components/table/useAmenityColumns";
...
const Page = () => {
  ...
  const { data: getAllAmenitiesRes } = useGetAllAmenitiesItemsQuery(
  ...
  const amenityListData = getAllAmenitiesRes?.data || [];
  const amenityListTotal = getAllAmenitiesRes?.totalRecords;
  const [selectedItems, setSelectedItems] = useState<number[]>([]);

  const amenityColumns = useAmenityColumns({});
  const mappedAmenityColumns = useCreation(() => {
    const { nameCombined1, price, minimumDeposit, stock, actionsState } = amenityColumns;
    return [
      { ...nameCombined1, width: 300 },
      { ...price, align: "right", width: 160, ellipsis: true },
      { ...minimumDeposit, align: "right", width: 160, ellipsis: true },
      { ...stock, align: "right", width: 120, ellipsis: true },
      { ...actionsState, align: "right", width: 190, ellipsis: true },
    ];
  }, [gSelectedProvider, amenityColumns]);
      ...
      <PageWrapper>
        ...
        >
          <Card
            ...
            extra={
              <Button
                type="primary"
                icon={<BsPlusLg />}
                href={`/supplier/store/${storeId}/amenities/create`}
              >
                {i18n["Thêm dịch vụ"]}
              </Button>
            }
            tabList={[{ key: "", tab: i18n["Tất cả dịch vụ"] }]}
          ...
          >
            ...
            {amenityListData?.length ? (
              <AmenityTableStyled
                showSorterTooltip={false}
                pagination={false}
                size="large"
                dataSource={amenityListData}
                columns={mappedAmenityColumns as any[]}
                rowSelection={{
                  columnWidth: 32,
