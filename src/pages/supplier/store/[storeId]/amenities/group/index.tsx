...
import useAmenitiesGroupColumns from "@/components/table/useAmenitiesGroupColumns";
...
const Page = () => {
  ...
  const amenitiesGroupListData = getAllAmenitiesGroupsRes?.data || [];
  const amenitiesGroupListTotal = getAllAmenitiesGroupsRes?.totalRecords;
  const [selectedItems, setSelectedItems] = useState<number[]>([]);

  const amenitiesGroupColumns = useAmenitiesGroupColumns();
  const mappedAmenitiesGroupColumns = useCreation(() => {
    const { name, description, actionsState } = amenitiesGroupColumns;
    return [
      { ...name, width: 300 },
      { ...description, ellipsis: true },
      { ...actionsState, align: "right", width: 190, ellipsis: true },
    ];
  }, [gSelectedProvider, amenitiesGroupColumns]);
          ...
          <Card
            ...
            extra={
              <Button
                type="primary"
                icon={<BsPlusLg />}
                href={`/supplier/store/${storeId}/amenities/group/create`}
              >
            ...
            <div className="header-wrapper">
              <div className="filters-wrapper">
                ...
                <Form.Item name="keyword" noStyle>
                  <FilterSearchInput placeholder={i18n["Tìm kiếm nhóm"]} />
                </Form.Item>
              </div>
            </div>

            {amenitiesGroupListData?.length ? (
              <AmenityTableStyled
                showSorterTooltip={false}
                pagination={false}
                size="large"
                dataSource={amenitiesGroupListData}
                columns={mappedAmenitiesGroupColumns as any[]}
                rowSelection={{
                  columnWidth: 32,
              ...
              <Empty
                ...
                description={
                  <Typography.Text ellipsis type="secondary">
                    {i18n["Không có nhóm dịch vụ nào"]}
                  </Typography.Text>
                }
              />
            )}
            ...
            <Affix offsetBottom={0.001}>
              <div className="footer" style={{ backgroundColor: colorBgBase }}>
                <div className="selected-wrapper">
                  {i18n["Đã chọn"]} <span>{selectedItems.length}</span> {i18n["Nhóm"].toLowerCase()}
                </div>
                {!!amenitiesGroupListTotal && (
                  <Pagination
                    showSizeChanger
                    total={amenitiesGroupListTotal}
