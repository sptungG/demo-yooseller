...
type TCertificateFormListProps = {
  name?: string | number | (string | number)[];
};

function CertificateFormList({ name = ["properties", "certificate"] }: TCertificateFormListProps) {
                ...
                title: (
                  <PropertiesFormListItemStyled>
                    ...
                    <Form.Item name={[field.name, "name"]} noStyle rules={[{ required: true }]}>
                      <Input
                        placeholder={`${i18n["Tên Chứng chỉ"]} ${index + 1}`}
                        variant="borderless"
                      />
                    </Form.Item>
                    <Form.Item
                    ...
                    >
                      <Input.TextArea
                        placeholder={`${i18n["Mô tả Chứng chỉ"]} ${index + 1}...`}
                        variant="borderless"
                        autoSize={{ minRows: 2 }}
                      />
                    </Form.Item>
