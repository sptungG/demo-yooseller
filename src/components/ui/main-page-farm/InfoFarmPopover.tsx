...
import { useGetProviderEcofarmByIdQuery } from "@/redux/query/farm.query";
...
type TInfoFarmPopoverProps = { storeId?: number; style?: React.CSSProperties };

const InfoFarmPopover = ({ storeId, style }: TInfoFarmPopoverProps) => {
  const uid = useId();
  ...
  return (
      ...
      content={
        <StyledContent suppressScrollX>
            ...
            <div className="header-wrapper">
              <div className="images-wrapper">
                <AvatarGroup key={uid + storeData.id + "images"} maxCount={3}>
                  {((storeData?.imageUrls || []) as any[]).map((item, index) => (
                    <Avatar
                      size={index === 0 ? 64 : 56}
              ...
              <div className="actions-wrapper">
                <Button
                  ...
                  href={`/supplier/farm/${String(storeData.id)}/edit`}
                ></Button>
              </div>
            </div>
                      ...
                      >
                        ...
                        <Descriptions.Item label={i18n["Đơn vị vận chuyển"]}>
                          <Flex gap={8} wrap="wrap" style={{ marginTop: 4 }}>
                            {(storeData?.carrierList as any[]).map((item, index) => (
                              <DeliveryMethodTag
          ...
          <div className="actions-bottom">
            <Link href={`/supplier/farm/${storeId}`}>
    ...
    >
      <StyledContainer style={{ border: open ? `1px solid ${colorPrimary}` : undefined, ...style }}>
        ...
        <Flex vertical style={{ maxWidth: 180 }}>
          <Typography.Title
            level={5}
            ellipsis
...
export default InfoFarmPopover;
