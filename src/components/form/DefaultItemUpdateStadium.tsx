...
type TDefaultItemUpdateStadiumProps = {};

const DefaultItemUpdateStadium = ({}: TDefaultItemUpdateStadiumProps) => {
  const uid = useId();
  const {
  ...
  const mappedInitialFormValues = useMemo(
    () => ({
      ...
      itemModel: {
        originalPrice: itemData?.modelList[0].originalPrice,
        currentPrice: itemData?.modelList[0].currentPrice,
      },
    }),
...
export default DefaultItemUpdateStadium;
