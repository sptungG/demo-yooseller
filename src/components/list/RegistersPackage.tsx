...
import useRegisterPackageFarmColumns from "../table/useRegisterPackageFarmColumns";

type TRegistersPackageProps = { packageId?: number; providerId?: number };

const RegistersPackage = ({ packageId, providerId }: TRegistersPackageProps) => {
  const uid = useId();
  ...
  const { data: getListEcofarmRegistersRes } = useGetListEcofarmRegisterByPartnerQuery(filterData, {
  ...
  const { nameCombined, actionsStatus, ecofarmType, totalPrice } = useRegisterPackageFarmColumns({
  ...
  const mappedColumns = [
    { ...nameCombined, width: 280 },
    { ...ecofarmType, width: 100, align: "right" },
    { ...totalPrice, width: 120, align: "right" },
...
export default RegistersPackage;
